        ...
        while len(blockchain.blockqueue) >= 1:
            ...
            try:
                ...
                if (blockchain.blockqueue[0].source == (str(node.server_ip)+" "+str(node.server_port))):
                    ...
                    #random_nonce_two_or_one = block.nonce*int(block.info.data)
                    
                    #print("test_nonce",random_nonce)
                    #print("test_nonce_1",random_nonce_two_or_one)
                    ...
                    #concent = "update||"+block.conv_block_to_str(1,0)+"||appendix||verification: "+ECC_1.sign_ECDSA_msg(block.get_blockhash_zpk(),node.server_port).decode()+"\\miner: "+str(node.server_port)+"\\finish_time: "+str(timestamp)
                    
                    concent = "update||"+block.conv_block_to_str_zpk(1,0)+"||appendix||verification: "+ECC_1.sign_ECDSA_msg(block.get_blockhash_zpk(),node.server_port).decode()+"\\miner: "+str(node.server_port)+"\\finish_time: "+str(timestamp)+"\\commitment:["
                    ...
                    #print("[no_1]")
                    #count += 1
                    blockchain.write_log_blockqueue(port = node.server_port)
                    #print("[problem_2]",blockchain.blockqueue[0].source)
            except:
                #print("[problem]")
                pass
    pass
        ...
        if(type(data) == dict):
            ...
            if data["msg"]["tag"] == "block":
                ...
                if data["msg"]["content"] == "FAKE":
                    ...
                    data["msg"]["content"].nonce = 1
                    ver_block_hash = (data["msg"]["appendix"]).encode()
                    compar_block_hash = data["msg"]["content"].get_blockhash_zpk()
                ...
                if data["msg"]["content"] == "FAKE":
                    ...
                    compar_blockchain_hash = data["msg"]["content"].get_blockhash_zpk()
                    if ECC_1.validate_signature(ver_blockchain_hash,compar_blockchain_hash,node_port) == "[double attack]":
                        ...
                        if ECC_1.validate_signature(ver_blockchain_hash,compar_blockchain_hash,node_port):
                            ...
                            print("[tttset]",commitment_cal().sequence(data["msg"]["appendix"]))
                            print("len_text",len(commitment_cal().sequence(data["msg"]["appendix"])))
                            data["msg"]["content"].nonce = 0
                            nonce_list = []
                            for a in range(((len(commitment_cal().sequence(data["msg"]["appendix"]))) -1)  // 2):    ###====###零知識
                                flag_zkp = True
                                flag_zkp_nine = False
                                for _ in range(9):
                                    if commitment_cal().sequence(data["msg"]["appendix"])[a][_] == 9:
                                        pass
                                    else:
                                        break
                                    if _ == 8:
                                ...
                                sender_solution_nonce = zero_knowledge_change.puzzle_columns(commitment_cal().sequence(data["msg"]["appendix"])[-1])
                                for _ in range(9):
                                    flag_zkp_count = False
                                    receiver_verification_commitment = zero_knowledge_change.puzzle_commitment(commitment_cal().sequence(data["msg"]["appendix"])[a], sender_solution_nonce[_])                                                       
                                    flag_zkp_nine_str = False
                                    for i in range(9):
                                        if commitment_cal().sequence(data["msg"]["appendix"])[a + (((len(commitment_cal().sequence(data["msg"]["appendix"]))) -1)  // 2)][i] == '9':
                                            pass
                                        else:
                                            break
                                        if i == 8:
                                    ...
                                    if (flag_zkp_nine_str):
                                        ...
                                        for b in range(9):
                                            if commitment_cal().sequence(data["msg"]["appendix"])[a + (((len(commitment_cal().sequence(data["msg"]["appendix"]))) -1)  // 2)][b] != receiver_verification_commitment[b]:
                                                break
                                            if b == 8:
                                                flag_zkp_count = True
                                        if(flag_zkp_count):
                            ...
                            print("[test_list]",nonce_list)
                            for _ in range(len(nonce_list)):
                                #print("[test_in]",nonce_list[len(nonce_list)-1-_])
                                #print("[test_in_nonce_before]",data["msg"]["content"].nonce)
                                data["msg"]["content"].nonce = (nonce_list[len(nonce_list)-1-_] + (data["msg"]["content"].nonce * 10))
                                #print("[test_in_nonce]",data["msg"]["content"].nonce)
                            
                            data["msg"]["content"].info.data = str(int(data["msg"]["content"].info.data) // data["msg"]["content"].nonce)
                            #print("[test_data]",data["msg"]["content"].info.data)
                            #print("[test_nonce]",data["msg"]["content"].nonce)
...
def new_block():
    ...
    info_str_hex = info_str.encode('utf-8')
    info_dec = int(info_str_hex.hex(), 16)
    print("[test1]",info_dec)
    ...
    if blockchain.chain[-1].info.pos == 1:
        ...
        block = Block(str(info_dec),Calculate.get_pos(),node.server_ip+" "+str(node.server_port))
    ...
    node.broadcast("block||"+block.conv_block_to_str_zpk(1,0)+"||appendix||"+ECC_1.sign_ECDSA_msg(block.get_blockhash_zpk(),node.server_port).decode())
